...
function News() {
  ...
  const [Local_token, setLocalToken] = useState(
    localStorage.getItem("token") || ""
  );
  const [Local_name, setLocalName] = useState(
    localStorage.getItem("username") || ""
  );
  const [Local_team, setLocalTeam] = useState(
            ...
            <div className="news_block">
              ...
              <div className="news_block_info">
                {club_internal.map((item, index) => (
                  <p key={index}>
                    <h3 className="inline">{index + 1}.</h3> {item.headline}{" "}
                  </p>
                ))}
              </div>
            </div>
            ...
            <div className="news_block">
              ...
              <div className="news_block_info">
                {player_idv.map((item, index) => (
                  <p key={index}>
                    {" "}
                    <h3 className="inline">{index + 1}.</h3>
                    {item.headline}{" "}
                  </p>
                ))}
              </div>
